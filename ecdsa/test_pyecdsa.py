...
from . import util
from .util import sigencode_der, sigencode_bytes_pair
from .util import sigdecode_der, sigdecode_bytes_pair
...
class ECDSA(unittest.TestCase):
    def test_basic(self):
        ...
        pub2 = VerifyingKey.from_bytes(pub.to_bytes())
    ...
    def test_lengths(self):
        ...
        priv = SigningKey.generate()
        pub = priv.get_verifying_key()
        self.assertEqual(len(pub.to_bytes()), default.verifying_key_length)
        ...
        for curve in (NIST192p, NIST224p, NIST256p, NIST384p, NIST521p):
            ...
            pub2 = VerifyingKey.from_bytes(pub1.to_bytes(), curve)
            self.assertEqual(pub1.to_bytes(), pub2.to_bytes())
            self.assertEqual(len(pub1.to_bytes()),
    ...
    def test_serialize(self):
        ...
        self.assertEqual(hexlify(priv1.to_bytes()),
                         hexlify(priv2.to_bytes()))
        ...
        self.assertEqual(hexlify(pub1.to_bytes()),
                             hexlify(pub2.to_bytes()))
    ...
    def test_nonrandom(self):
        ...
        self.assertEqual(hexlify(priv1.get_verifying_key().to_bytes()),
                             hexlify(priv2.get_verifying_key().to_bytes()))
    ...
    def test_privkey_bytes(self):
        priv1 = SigningKey.generate()
        s1 = priv1.to_bytes()
        self.assertEqual(type(s1), bytes)
        self.assertEqual(len(s1), NIST192p.baselen)
        priv2 = SigningKey.from_bytes(s1)
        self.assertTruePrivkeysEqual(priv1, priv2)

        s1 = priv1.to_pem()
        self.assertEqual(type(s1), str)
        self.assertTrue(s1.startswith("-----BEGIN EC PRIVATE KEY-----"))
        self.assertTrue(s1.strip().endswith("-----END EC PRIVATE KEY-----"))
    ...
    def test_pubkey_bytes(self):
        ...
        pub2 = VerifyingKey.from_bytes(s1)
        ...
        pub2 = VerifyingKey.from_bytes(s1, curve=NIST256p)
        self.assertTruePubkeysEqual(pub1, pub2)
        ...
        pem = pub1.to_pem()
        self.assertEqual(type(pem), str)
        self.assertTrue(pem.startswith("-----BEGIN PUBLIC KEY-----"), pem)
        self.assertTrue(pem.strip().endswith("-----END PUBLIC KEY-----"), pem)
    ...
    def test_signature_bytes(self):
        ...
        sig = priv1.sign(data, sigencode=sigencode_bytes_pair)
        ...
        self.assertTrue(pub1.verify(sig, data, sigdecode=sigdecode_bytes_pair))
    ...
    def test_hashfunc(self):
        ...
        sig = sk.sign(data)
        vk = VerifyingKey.from_bytes(sk.get_verifying_key().to_bytes(),
                                      curve=NIST256p, hashfunc=sha256)
        self.assertTrue(vk.verify(sig, data))

        sk2 = SigningKey.generate(curve=NIST256p)
        sig2 = sk2.sign(data, hashfunc=sha256)
        vk2 = VerifyingKey.from_bytes(sk2.get_verifying_key().to_bytes(),
                                       curve=NIST256p, hashfunc=sha256)
        self.assertTrue(vk2.verify(sig2, data))

        vk3 = VerifyingKey.from_bytes(sk.get_verifying_key().to_bytes(),
                                       curve=NIST256p)
...
class DER(unittest.TestCase):
    ...
    def test_number(self):
        ...
        #self.assertEqual(der.encode_number(155), "\x81\x9b")
...
class Util(unittest.TestCase):
    def test_trytryagain(self):
        ...
        self.assertEqual("%x"%(tta("seed", NIST224p.order)),
